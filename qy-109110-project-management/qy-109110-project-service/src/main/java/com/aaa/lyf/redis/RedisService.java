package com.aaa.lyf.redis;

import com.aaa.lyf.utils.JSONUtils;
...
import static com.aaa.lyf.staticproperties.RedisProperties.*;
 ...
 *     是固定值，有两个值
 *     “nx”：
 *          如果redis中的key不存在，则就可以存储
 *          如果redis中的key存在，则不存数据
 *      “xx”
 *          如果redis中的key存在，则直接覆盖
 *          如果redis中的key不存在，则不存数据
 *  eg:
 *    研发组有两个人   一个张三  一个李四
 *    张三负责商品管理的代码编写
 *    李四负责订单管理
 *    张三李四因为因为数据量大都会用到redis
